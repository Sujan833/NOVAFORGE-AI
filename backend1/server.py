...
from generation_pipeline import (
    ...
    generate_contextual_image_urls,
...
def _infer_site_archetype(name: str, desc: str, project_type: str) -> str:
    ...
    booking_markers = ("appointment", "appointments", "schedule", "slot", "service provider", "provider availability", "booking system")
    travel_markers = ("travel", "trip", "tour", "hotel", "flight", "destination", "vacation")
...
def _default_hero_subtitle(archetype: str) -> str:
    mapping = {
        ...
        "booking": "Register users, manage provider availability, and schedule appointments through a smooth booking flow.",
...
def _is_generation_quality_ok(html: str, features, project_type: str = "") -> bool:
    ...
    if "Authentication" in features_set:
        has_signup = ("signup-form" in low) or ('data-auth="signup"' in low) or ("data-auth='signup'" in low)
        has_login = ("login-form" in low) or ('data-auth="login"' in low) or ("data-auth='login'" in low)
        if not (has_signup and has_login):
            return False
...
def _instruction_allows_broad_changes(instruction: str) -> bool:
    text = (instruction or "").lower()
    broad_markers = (
        "redesign",
        "rewrite all",
        ...
        "full overhaul",
        "rebuild everything",
        "change whole website",
        "replace everything",
    )
    return any(marker in text for marker in broad_markers)


def _line_change_ratio(old_code: str, new_code: str) -> float:
    ...
    if not old_lines or not new_lines:
        return 1.0
    return 1.0 - SequenceMatcher(None, old_lines, new_lines).ratio()


def _is_unscoped_rewrite(
    instruction: str,
    mode: str,
    old_html: str,
    old_css: str,
    ...
    if _instruction_allows_broad_changes(instruction):
        return False

    thresholds = {
        "bugfix": {"html": 0.20, "css": 0.25, "js": 0.25, "backend": 0.10},
        "replace": {"html": 0.35, "css": 0.35, "js": 0.35, "backend": 0.20},
        "refactor": {"html": 0.50, "css": 0.50, "js": 0.55, "backend": 0.35},
        "ui": {"html": 0.60, "css": 0.75, "js": 0.35, "backend": 0.02},
        "auth": {"html": 0.50, "css": 0.50, "js": 0.65, "backend": 0.65},
        "data": {"html": 0.45, "css": 0.40, "js": 0.65, "backend": 0.80},
        "feature": {"html": 0.55, "css": 0.55, "js": 0.65, "backend": 0.50},
    }
    limit = thresholds.get(mode, thresholds["feature"])

    changed = {
        "html": _line_change_ratio(old_html, new_html),
        "css": _line_change_ratio(old_css, new_css),
        "js": _line_change_ratio(old_js, new_js),
        "backend": _line_change_ratio(old_backend, new_backend),
    }
    return any(changed[key] > limit[key] for key in ("html", "css", "js", "backend"))
...
Make minimal scoped edits only for the user request.
...
- If a file is not needed for this request, return it unchanged exactly.
- Do not refactor, rename, reformat, reorder, or clean unrelated code.
- Do not remove existing sections, endpoints, handlers, IDs, classes, or functions unless explicitly requested.
...
def _copilot_response_ok(new_html: str, new_css: str, new_js: str, new_backend: str) -> bool:
    if not new_html or not new_css or not new_js or new_backend is None:
        return False
    low_html = new_html.lower()
    if "<html" not in low_html and "<!doctype" not in low_html:
        return False
    if "```" in new_html or "```" in new_css or "```" in new_js or "```" in new_backend:
        return False
...
"""
    ...
    if html:
        available_images = generate_contextual_image_urls(name, desc, analysis, ui_plan)
...
def _run_copilot_modify(project: str, instruction: str, mode: str = "auto"):
    ...
    if not _copilot_response_ok(new_html, new_css, new_js, new_backend):
        return {"success": False, "error": "Copilot returned incomplete/invalid update"}, 500
    if _is_unscoped_rewrite(
        instruction,
        effective_mode,
        html,
        css,
        ...
        return {
            "success": False,
            "error": "Copilot attempted broad unrelated changes. Ask for a full redesign to allow major rewrites."
        }, 400
